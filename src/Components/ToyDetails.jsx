...
const ToyDetails = () => {
  ...
  if (!toy) {
    return <div className="text-center py-16 text-2xl font-semibold">Toy not found!</div>;
  }

  return (
    <div className="container mx-auto px-4 my-8">
      {/* Back button to navigate to home */}
      <button onClick={() => navigate('/')} className="text-indigo-600 underline text-lg mb-6">
        ⬅ Back to Home
      </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
        <div className="relative">
          ...
          <div className="absolute bottom-4 left-4 text-white bg-black bg-opacity-50 px-4 py-2 rounded-lg">
            <h2 className="text-2xl font-semibold">{toy.toyName}</h2>
            <p className="text-lg font-medium">Price: ${toy.price}</p>
            <p className="text-sm">Rating: {toy.rating}⭐</p>
          </div>
        </div>
        <div className="space-y-6">
          <h2 className="text-3xl font-semibold text-gray-800">{toy.toyName}</h2>
          ...
          {/* Form to Try Now */}
          <h3 className="text-2xl font-semibold mt-4 text-gray-800">Try Now</h3>
          {message && <p className="text-green-600">{message}</p>} {/* Success message */}
          <form onSubmit={handleFormSubmit} className="space-y-4 mt-4">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-indigo-600 to-teal-500 text-white py-3 rounded-lg hover:bg-gradient-to-l transition-all duration-200"
            >
              Try Now
            </button>
          </form>
